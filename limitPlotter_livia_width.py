...
if Method=="HybridNew":
  ...
  EXPwidths = OBSwidths[:]
  for w in EXPwidths:     
      if w == 0.10: EXPfiles.append(ROOT.TFile(EXPName+"_w0.10_COMB."+Method+".mH"+options.mass+".root"))
      if w == 2.00: EXPfiles.append(ROOT.TFile(EXPName+"_w2.00_COMB."+Method+".mH"+options.mass+".root"))
      if w == 5.00: EXPfiles.append(ROOT.TFile(EXPName+"_w5.00_COMB."+Method+".mH"+options.mass+".root"))
      if w == 7.00: EXPfiles.append(ROOT.TFile(EXPName+"_w7.00_COMB."+Method+".mH"+options.mass+".root"))
      if w == 10.00: EXPfiles.append(ROOT.TFile(EXPName+"_w10.00_COMB."+Method+".mH"+options.mass+".root"))
...
if not options.expectedOnly:
    for w in OBSwidths:     
        if w == 0.10: OBSfiles.append(ROOT.TFile(EXPName+"_w0.10_COMB."+Method+".mH"+options.mass+".root"))
        if w == 2.00: OBSfiles.append(ROOT.TFile(EXPName+"_w2.00_COMB."+Method+".mH"+options.mass+".root"))
        if w == 5.00: OBSfiles.append(ROOT.TFile(EXPName+"_w5.00_COMB."+Method+".mH"+options.mass+".root"))
        if w == 7.00: OBSfiles.append(ROOT.TFile(EXPName+"_w7.00_COMB."+Method+".mH"+options.mass+".root"))
        if w == 10.00: OBSfiles.append(ROOT.TFile(EXPName+"_w10.00_COMB."+Method+".mH"+options.mass+".root"))
...
def MakeMlfPlot(MG):
    legend=ROOT.TLegend(0.55,0.15,0.89,0.45) #0.15-0.3
...
def MakeLimitPlot(MG):
  ...
  #  leg=ROOT.TLegend(0.2, 0.2,0.59, 0.5) #
    ...
    dummyHist.GetXaxis().SetTitle("#Gamma_{X}/m_{X} [%] ")
